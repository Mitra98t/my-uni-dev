// VERSIONE IN CUI LO SCRITTORE DA SEMPRE PRECEDENZA AGLI SCRITTORI:
//
// in questa versione non appena uno scrittore entra fa sempre passare 
//  gli altri scrittori in attesa
// 
//
...
void startRead() {
  ...
  waitingReaders++;
  // il nuovo lettore se ci sono scrittori in attesa non entra
...
void doneWriter() {
  ...
  if (waitingWriters > 0) {      // passano sempre prima gli scrittori
    pthread_cond_signal(&writerGo);
  } else {
    pthread_cond_broadcast(&readerGo); // riattiva tutti i lettori
  }
