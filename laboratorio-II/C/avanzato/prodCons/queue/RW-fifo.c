...
void startWrite(int id) {
  ...
  if (state >= 0) {  // c'e' uno scrittore oppure dei lettori
    ...
    do {
      pthread_cond_wait(&o->ord, &mutex);  // vengo svegliato con stato=-1 per avere precedenza su eventuali lettori che arrivano prima che riprendo la lock
    } while(state >= 0);    
    assert(state == -1);
    ordering_t* otmp = pop(orderingQ);
    assert(o == otmp);
